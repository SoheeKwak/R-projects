...
[Workspace loaded from D:/r_work/.RData]
...
class(mylist[[3]][[1]])
#리스트 참조 : []를 사용하면 리스트를, [[]]를 사용하여 벡터를 구함
mylist[[3]][[1]][3]
#unlist함수 : 리스트를 벡터형식으로 변환해주는 함수(사용 주의)
myvector<-c(1:6,'a')
mylist<-list(1:6,'a')
class(myvector)
unlist(mylist)
...
#단어들을 조합하여 문장을 구성, 문장->문단, 문단->문서...작업시 위와 같은 형식으로 처리
...
my2sentence<-c("Learing R is so interesting",
